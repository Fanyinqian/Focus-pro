...
# ZFProDescriptions - 高级定义列表

## 何时使用

高级描述列表组件，提供一个更加方便快速的方案来构建描述列表。

ZFProDescriptions 的诞生是为了解决项目中需要写很多 Descriptions 的样板代码的问题，所以在其中做了封装了很多常用的逻辑。在 React 中写一个 Descriptions 免不了需要定义一些雷同的属性。所以 ZFProDescriptions 默认封装了请求网络，columns 列展示的逻辑。

比如 ZFProDescriptions 封装了请求网络的行为，ZFProDescriptions 会将 props.params 中的数据默认带入到请求中，如果接口恰好与我们的定义相同，实现一个查询会非常简单。
...
对与请求回来的结果的 ZFProDescriptions 也有一些约定，类型如下：
...
ZFProDescriptions 支持了和 Table 相同的 dataSource
...
| request        | 请求数据，不设置 columns 时 ZFProDescriptions.Item 需设置对应的 dataIndex                         | -                                    | -            |
...
| dataIndex | 返回数据的 key 与 ZFProDescriptions 的 request 配合使用，用于配置式的定义列表 | `React.Text` \| `React.Text[]`                               | -      |
...
参考 ZFProTable [操作类型 action](/pro-table#操作类型-action)
