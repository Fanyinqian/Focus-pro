...
import type { CommonFormProps } from '@ant-design/pro-form/es/BaseForm';
import type { ProFormGridConfig } from '@ant-design/pro-form/es/typing';
import type {
  ProCoreActionType,
  ProSchema,
...
export type ProFormPropsType<T, ValueType = 'text'> =
  | ((
      ...
      columns: ZFProFormColumnsType<T, ValueType>[];
    })
  | ({
...
/** ProForm 的特色 layout */
export type ZFProFormLayoutType = ProFormPropsType<any>['layoutType'];
...
export type ZFProFormColumnsType<T = any, ValueType = 'text'> = ProSchema<
  T,
  ExtraProColumnType & {
    index?: number;
    /**
...
export type ZFFormSchema<T = Record<string, any>, ValueType = 'text'> = {
  title?:
    | React.ReactNode
    | ((
        schema: ZFProFormColumnsType<T, ValueType>,
        type: 'form',
        dom: React.ReactNode,
      ) => React.ReactNode);
